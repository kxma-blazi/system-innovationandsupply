...
function sendStockChart(chatId) {
  ...
  const data = labels.map((d) => stats[d].quantity);

  const chart = Charts.newBarChart()
    .setTitle("Stock Quantity by Department")
    .addRange(
...
function sendStockChart(chatId) {
  try {
    const stats = getWithdrawalStats(); // ข้อมูล 7 วัน
    ...
    // สร้าง DataTable สำหรับ Charts Service
    const data = [["Date", "Withdrawals"]];
    for (let i = 0; i < labels.length; i++) {
      data.push([labels[i], Number(values[i])]); // แปลงเป็น number เผื่อเป็น string
    }
    ...
    // แปลงเป็น PNG Blob
    const blob = chart.getAs("image/png").setName("withdrawal_chart.png");

    // ส่งเข้า Telegram (ต้องใช้ method ที่รองรับ blob)
    sendTelegramPhotoBlob(chatId, blob, "📊 กราฟการเบิกย้อนหลัง 7 วัน");
  } catch (e) {

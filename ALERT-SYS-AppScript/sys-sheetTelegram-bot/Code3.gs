  ...
  try {
    ...
    if (data.callback_query) {
      ...
      if (callbackData === "/allstock") {
        ...
        responseText = lowStock(); // เรียกฟังก์ชันที่สร้างใหม่
      } else if (callbackData === "/report") {
        responseText = stockDashboard(); // เชื่อมไปที่ dashboard
      } else if (callbackData === "/report") {
        responseText = report(); 
...
function sendControlPanel(chatId) {
  ...
  const payload = {
    ...
    "text": "🎮 <b>SYSTEM INNOOVATION AND SUPPLY</b>\nเลือกรายการที่ต้องการดำเนินการด้านล่าง:",
    "parse_mode": "HTML",
...
// ฟังก์ชันสำหรับดึงรายการสินค้าใกล้หมด (คืนค่าเป็นข้อความ)
function lowStock() {
  const data = getSheet().getDataRange().getValues();
  let results = "";
  let count = 0;
  ...
  for (let i = 1; i < data.length; i++) {
    if (Number(data[i][4]) <= LOW_STOCK_LIMIT) {
      results += `• [${data[i][0]}] ${data[i][2]} (คงเหลือ: <b>${data[i][4]}</b>)\n`;
      count++;
    }
  }
  
  return count > 0 
    ? `⚠️ <b>รายการสินค้าใกล้หมด (${count} รายการ)</b>\n\n${results}` 
    : "✅ สต็อกปกติทุกรายการครับ";
}

// ฟังก์ชันสรุปรายงานแบบข้อความ (สำหรับปุ่ม /report)
function report() {
  const data = getSheet().getDataRange().getValues();
  let totalQty = 0;
  let lowItems = 0;
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (Number(data[i][4]) <= LOW_STOCK_LIMIT) lowItems++;
  }
  
  return `📊 <b>สรุปภาพรวมคลังสินค้า</b>\n` +
         `━━━━━━━━━━━━━━━\n` +
         `📦 รายการสินค้าทั้งหมด: ${data.length - 1} รายการ\n` +
         `🔢 จำนวนชิ้นรวม: ${totalQty} ชิ้น\n` +
         `⚠️ ของใกล้หมด: ${lowItems} รายการ\n` +
         `🕒 อัปเดตเมื่อ: ${Utilities.formatDate(new Date(), "GMT+7", "dd/MM/yyyy HH:mm")}`;
}
...
function helpMenu() {
  return `📦 <b>SYSTEM INNOVATION AND SUPPLY</b>
📚 <b>คู่มือการใช้งาน</b>
━━━━━━━━━━━━━━━━━━

🧭 <b>เริ่มต้นใช้งาน</b>
• <code>/menu</code>
เปิดเมนูควบคุมหลัก (สามารถกดปุ่มแทนการพิมพ์คำสั่งได้)

━━━━━━━━━━━━━━━━━━
🔎 <b>1. ตรวจสอบสินค้า</b>

• <code>/stock CODE</code>
ดูจำนวนสต็อกของสินค้า

ตัวอย่าง
<code>/stock Q001</code>

• <code>/search NAME</code>
ค้นหาสินค้าด้วยชื่อ

ตัวอย่าง
<code>/search cable</code>
...
หมายถึง  
เติมสินค้า Q001 จำนวน 50 ชิ้น

━━━━━━━━━━━━━━━━━━
📤 <b>3. เบิกสินค้าออกจากคลัง</b>
...
หมายถึง  
แผนก CBR เบิกสินค้า Q001 จำนวน 5 ชิ้น
...
• <code>/history</code>
ดูประวัติการเบิก/เติมล่าสุด

• <code>/exportpdf</code>
ดาวน์โหลดรายงานสต็อกเป็น PDF

━━━━━━━━━━━━━━━━━━
💡 <b>Tips</b>

คุณสามารถ  
👉 กดปุ่มจาก <code>/menu</code>  
แทนการพิมพ์คำสั่งได้

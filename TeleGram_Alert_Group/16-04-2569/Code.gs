...
function writeLog(user, act, code, qty, note, rem) {
  try {
    ...
    // --- ส่วนของ AI Memory ---
    let finalNote = note;
    // ถ้ามีการเบิกสินค้า ให้ AI ช่วยวิเคราะห์พฤติกรรมสั้นๆ
    if (act.includes("Withdraw") || act.includes("เบิก")) {
      const aiSummary = askAiToSummarize(
        `ผู้ใช้: ${user}, ทำกิจกรรม: ${act}, สินค้า: ${code}, จำนวน: ${qty}, หมายเหตุเดิม: ${note}`,
      );
      if (aiSummary) {
        finalNote = aiSummary; // ใช้ข้อความที่ AI สรุปแทน
      }
    }

    // บันทึกข้อมูลลงในแถวถัดไปที่ว่างจริงๆ
    const logData = [new Date(), user, act, code, qty, finalNote, rem];
...
function getWithdrawalStats() {
  try {
    ...
    if (!logSheet) return { daily: {}, total: 0 };

    const lastRow = logSheet.getLastRow();

    // ✅ ดึงแค่ 200 แถวล่าสุด (เร็วขึ้นเยอะ)
    const startRow = Math.max(2, lastRow - 200);

    const data = logSheet
      .getRange(startRow, 1, lastRow - startRow + 1, 6)
      .getValues();
...
function doPost(e) {
  try {
    ...
    if (!text) {
      sendTelegram(chatId, "⚠️ ไม่มีข้อความ");
      return;
    }
    ...
    // ===== 📊 CHART (สำคัญ) =====
      ...
      sendTelegram(chatId, "⏳ กำลังสร้างกราฟ...");
      sendSummaryReport(chatId);
    }
    ...
    // ===== RESTOCK =====
      ...
      if (args.length < 3) {
        sendTelegram(chatId, "⚠️ ใช้: /restock [รหัส] [จำนวน]");
      } else {
        let qty = parseInt(args[2]) || 0;
    ...
    // ===== เบิก =====
      ...
      if (!input) {
        sendTelegram(chatId, `⚠️ ใช้: ${cmd} [ID/SN]`);
        return;
      }
    ...
    // ===== DEFAULT =====
      ...
      sendTelegram(chatId, "❌ ไม่รู้จักคำสั่ง");
    }
  } catch (err) {
    ...
    try {
      ...
      const chatId = data?.message?.chat?.id;
      if (chatId) {
        sendTelegram(chatId, "❌ ระบบมีปัญหา: " + err.message);
      }
    } catch (e2) {}
  }
}
...
function withdrawBySerial(serial, colIndex, deptName, user) {
  const lock = LockService.getScriptLock();
  try {
    // 1. ล็อกระบบ 15 วินาที ป้องกันการเบิกชนกัน (Race Condition)
    lock.waitLock(15000);
    ...
    const targetSerial = String(serial).trim().toLowerCase(); // ทำเป็นตัวเล็กเพื่อเทียบง่ายๆ

    let foundRow = -1;
    let takerInfo = null;
    let matCode = "";

    // 2. ค้นหา Serial ทั้งในช่อง Available (Col B) และ Taken (Col C)
    for (let i = 0; i < sData.length; i++) {
      const colB = String(sData[i][1] || "")
        .trim()
        .toLowerCase();
      const colC = String(sData[i][2] || "")
        .trim()
        .toLowerCase();

      if (colC === targetSerial) {
        takerInfo = {
          matCode: sData[i][0],
          date: sData[i][3],
          name: sData[i][5],
...
📦 ${takerInfo.matCode}`;
    ...
    // 3. อัปเดต SERIAL_DATA (ย้ายจาก Col B ไป Col C)
    const now = new Date();
    serialSheet.getRange(foundRow, 2).clearContent(); // ลบจากช่องว่าง
    serialSheet.getRange(foundRow, 3).setValue(serial); // ใส่ในช่องเบิกแล้ว
    ...
    // 4. ตัดสต็อกในหน้า STOCK
    const stockSheet = ss.getSheetByName("STOCK");
    const stockLastRow = stockSheet.getLastRow();
    const stockData = stockSheet
      .getRange(2, 1, stockLastRow - 1, 11)
      .getValues();

    let newTotal = 0;
    let foundStock = false;
    const targetMatCode = String(matCode).toLowerCase().trim();
    ...
    for (let j = 0; j < stockData.length; j++) {
      ...
      if (stockCode === targetMatCode) {
        const currentTotal = Number(stockData[j][5]) || 0; // Col F (Index 5)
        ...
        stockSheet.getRange(rowIndex, 6).setValue(newTotal); // อัปเดต Total
        stockSheet.getRange(rowIndex, colIndex + 1).setValue(newDeptQty); // อัปเดตยอดแผนก
    ...
    // สำคัญมาก: ต้องปลดล็อกเพื่อให้คนอื่นทำงานต่อได้
    lock.releaseLock();
  }
}
...
function sendStockChart(chatId) {
  try {
    ...
    // ✅ สร้าง DataTable แบบถูกต้อง
    const dataTable = Charts.newDataTable()
      .addColumn(Charts.ColumnType.STRING, "Date")
      .addColumn(Charts.ColumnType.NUMBER, "Withdrawals");

    for (let i = 0; i < labels.length; i++) {
      dataTable.addRow([labels[i], Number(values[i]) || 0]);
    }

    // ✅ สร้างกราฟ (เพิ่ม option กัน error)
    const chart = Charts.newBarChart()
      .setTitle("Withdrawal Last 7 Days")
      .setXAxisTitle("Date")
      .setYAxisTitle("Amount")
      .setDimensions(700, 300)
      .setDataTable(dataTable.build()) // 🔥 สำคัญมาก (ต้อง .build())
      .build();

    const blob = chart.getBlob().setName("chart.png");

    sendTelegramPhotoBlob(chatId, blob);
  } catch (e) {
    Logger.log(e); // 👈 เอาไว้ดูใน Apps Script
...
function sendTelegramPhotoBlob(chatId, blob) {
  try {
    ...
    const formData = {
      chat_id: String(chatId), // 🔥 กัน error type
      photo: blob,
    };
    ...
    const options = {
      method: "post",
      payload: formData,
      muteHttpExceptions: true, // 🔥 สำคัญ (จะได้เห็น error จริง)
    };

    const res = UrlFetchApp.fetch(url, options);
    const text = res.getContentText();

    Logger.log("Telegram response: " + text); // 👈 เอาไว้ debug
    ...
    Logger.log("SendPhoto Error: " + e.message);
    sendTelegram(chatId, "❌ ส่งรูปไม่สำเร็จ\n" + e.message);
  }
}
...
function updateStock(rowNum, newData) {
  try {
    ...
    // ดึงค่าเดิม 11 คอลัมน์ (A-K)
    const oldValues = sheet.getRange(rowNum, 1, 1, 11).getValues()[0];

    // สร้าง Array ใหม่โดยเรียงตามโครงสร้างชีตจริง
    const newValues = [
      oldValues[0], // A: ลำดับ
      newData.code !== undefined ? newData.code : oldValues[1], // B: Material Code
      oldValues[2], // C: (ว่าง/รูปภาพ)
      newData.desc !== undefined ? newData.desc : oldValues[3], // D: Description
      oldValues[4], // E: Unit
      newData.qty !== undefined ? newData.qty : oldValues[5], // F: Total Qty
      newData.cbr !== undefined ? newData.cbr : oldValues[6], // G: CBR
      newData.ccs !== undefined ? newData.ccs : oldValues[7], // H: CCS
      newData.sko !== undefined ? newData.sko : oldValues[8], // I: SKO
      newData.ryg !== undefined ? newData.ryg : oldValues[9], // J: RYG
      newData.trt !== undefined ? newData.trt : oldValues[10], // K: TRT
    ];

    // เขียนทับลงไปทั้งแถว (11 คอลัมน์)
    sheet.getRange(rowNum, 1, 1, 11).setValues([newValues]);

    // บันทึก Log และส่ง Telegram (ใช้โค้ดเดิมของคุณได้เลย)
    writeLog(
...
function sendSummaryReport(chatId) {
  try {
    sendTelegram(chatId, "⏳ กำลังสร้างรายงาน...");

    const stats = getWithdrawalStats();

    if (!stats || !stats.daily) {
      sendTelegram(chatId, "❌ ไม่มีข้อมูล");
      return;
    }

    const daily = stats.daily;
    const labels = Object.keys(daily);
    const values = Object.values(daily).map(Number);

    if (labels.length === 0) {
      sendTelegram(chatId, "⚠️ ไม่มีข้อมูลย้อนหลัง 7 วัน");
      return;
    }
...
🗓 รายวัน:
${labels.map((d, i) => `- ${d}: ${values[i]}`).join("\n")}
`;
    ...
    labels.forEach((l, i) => {
      dataTable.addRow([l, values[i]]);
    });

    const chart = Charts.newLineChart() // 🔥 ใช้ line chart สวยกว่า
      .setTitle("Withdrawal Last 7 Days")
    ...
    sendTelegram(chatId, "❌ สร้างรายงานไม่สำเร็จ\n" + e.message);
  }
}

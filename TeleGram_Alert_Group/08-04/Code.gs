...
function getStockRows(q) {
  ...
  const userEmail = Session.getActiveUser().getEmail() || "Guest";
  // บันทึก Log เมื่อมีการโหลดหรือค้นหาข้อมูล
  const actionDetail = q ? "ค้นหา: " + q : "โหลดข้อมูลทั้งหมด";
  writeLog(userEmail, "ดึงข้อมูล Stock", "ALL", 0, actionDetail, "-");
...
function writeLog(user, act, code, qty, note, rem) {
  try {
    ...
    // ถ้าไม่มีหน้า Logs ให้สร้างใหม่พร้อม Header
    if (!sheet) {
    ...
    // บังคับให้เขียนข้อมูลทันที
    SpreadsheetApp.flush();

    Logger.log("✅ บันทึก Log สำเร็จ: " + act + " [" + code + "]");
  } catch (e) {

...
function getStockRows() {
  ...
  // ถ้าต้องการใช้ cache ให้เปลี่ยน false => true
...
function withdrawBySerial(serial, colIndex, deptName, user) {
  ...
  // ✅ อ่านเฉพาะช่วงจริง (เร็วกว่า getDataRange)
  ...
  for (let i = 0; i < sData.length; i++) {
    ...
    // ✅ ยังว่าง
    if (colB === serial) {
      foundRow = i + 2; // +2 เพราะเริ่มจากแถว 2
      matCode = sData[i][0];
      break;
    }
  }
...
function sendStockChart(chatId) {
  try {
    const stats = getWithdrawalStats(); // ใช้ข้อมูล 7 วัน
    ...
    const chartUrl =
      ...
      "&chl=" +
      labels.join("|") +
      "&chtt=Withdrawal+Last+7+Days";

    // ส่งเป็นรูป
    sendTelegramPhoto(chatId, chartUrl, "📊 กราฟการเบิกย้อนหลัง 7 วัน");
  } catch (e) {

...
function doPost(e) {
  try {
    ...
    switch (command) {
      ...
      // ── แผนกเบิก (เดิม) ──
      case "/dp-cbr": handleDeptWithdraw(chatId, args, 5, "CBR", user); break;
      case "/dp-ccs": handleDeptWithdraw(chatId, args, 6, "CCS", user); break;
      case "/dp-sko": handleDeptWithdraw(chatId, args, 7, "SKO", user); break;
      case "/dp-ryg": handleDeptWithdraw(chatId, args, 8, "RYG", user); break;
      case "/dp-trt": handleDeptWithdraw(chatId, args, 9, "TRT", user); break;
...
// ============================================================
// 3. MAIN MENU (สวยขึ้น - Grid Layout)
// ============================================================
...
function handleCallback(query) {
  ...
  // ปุ่มเบิกจาก inline ของรายการ: wd_[ID]_[dept]
  if (data.startsWith("wd_")) {
    const parts = data.split("_");
    const code = parts[1], dept = parts[2].toUpperCase();
    const colMap = { CBR: 5, CCS: 6, SKO: 7, RYG: 8, TRT: 9 };
    const col = colMap[dept] || 5;
    const result = withdraw(code, 1, col, dept, user, "STOCK");
    sendMsg(chatId, result); return;
  }

  // ปุ่มดูรายละเอียดรายการ: info_[ID]
...
function restock(code, qty, user, targetSheetName) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0].toString().toLowerCase() === code.toLowerCase()) {
      const newQty = (Number(data[i][4]) || 0) + qty;
      sheet.getRange(i + 1, 5).setValue(newQty);
...
function sendSearchResult(chatId, keyword) {
  ...
  // ค้นหาจากชื่อ / ID / แผนก
  const deptColMap = { cbr: 5, ccs: 6, sko: 7, ryg: 8, trt: 9 };
  ...
  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    if (!row[0]) continue;
    const matchName = row[2].toString().toLowerCase().includes(kw);
  ...
  const top = results.slice(0, 8); // แสดงสูงสุด 8 รายการ
  let msg = `🔍 *ผลการค้นหา "${keyword}"* (${results.length} รายการ)\n━━━━━━━━━━━━━━━\n\n`;
  top.forEach(r => {
    const alert = Number(r[4]) <= LOW_STOCK_LIMIT ? " ⚠️" : " ✅";
    msg += `🔹 \`${r[0]}\` *${r[2].toString().substring(0,30)}*\n   คงเหลือ: *${r[4]}* ชิ้น${alert}\n\n`;
  });
  if (results.length > 8) msg += `_...และอีก ${results.length - 8} รายการ_\n`;

  // สร้างปุ่ม inline สำหรับแต่ละผลลัพธ์
...
// ============================================================
// 7. STOCK DETAIL + QUICK ACTION BUTTONS
// ============================================================
function sendStockDetail(chatId, code) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0].toString().toLowerCase() === code.toLowerCase()) {
      const r = data[i];
      const alert = Number(r[4]) <= LOW_STOCK_LIMIT ? "⚠️ ใกล้หมด" : "✅ ปกติ";
...
function getLowStock(sheetName) {
  const data = getSpreadsheet().getSheetByName(sheetName).getDataRange().getValues();
  const list = data.filter((r, i) => i > 0 && Number(r[4]) <= LOW_STOCK_LIMIT)
                   .map(r => `⚠️ \`${r[0]}\` ${r[2].toString().substring(0,25)} *(${r[4]})*`);
...
function sendAllStock(chatId, page, sheetName) {
  ...
  current.forEach(r => {
    const alert = Number(r[4]) <= LOW_STOCK_LIMIT ? "⚠️" : "✅";
    msg += `${alert} \`${r[0]}\` *${r[2].toString().substring(0, 20)}*\n   QTY: *${r[4]}* ชิ้น\n\n`;
  });
  const nav = [];
...
// รันทุกวันตอน 08:00 — ตั้ง Time Trigger ที่ sendDailyReport
...
// รันทุกวันจันทร์ตอน 08:00 — ตั้ง Time Trigger ที่ sendWeeklyReport
  ...
  try {
    ...
    if (sheet) {
      ...
      for (let i = 1; i < Math.min(data.length, 51); i++)
        ctx += `- ${data[i][2]} (ID: ${data[i][0]}) คงเหลือ: ${data[i][4]}\n`;
    }
...
// ============================================================
// 13. DASHBOARD DATA (สำหรับ Web App index.html)
// ============================================================
function getStockData() {
  try {
    ...
    const items = sheet.getDataRange().getValues().slice(1)
      ...
      .map(r => ({
        id: String(r[0]||""), materialCode: String(r[1]||""), description: String(r[2]||""),
        sn: String(r[3]||""), qty: Number(r[4])||0,
        dpCBR: Number(r[5])||0, dpCCS: Number(r[6])||0, dpSKO: Number(r[7])||0, dpRYG: Number(r[8])||0,
    ...
    const top10 = [...items].sort((a,b) => b.qty-a.qty).slice(0,10)
      .map(r => ({ label: r.description.substring(0,25)+(r.description.length>25?"…":""), value: r.qty }));
    const deptData = {
...
// ============================================================
// 14. SET WEBHOOK (รันครั้งเดียวหลัง Deploy)
// ============================================================
function setWebhook() {
  const WEB_APP_URL = "https://script.google.com/macros/s/AKfycbzdtBkRglc5Uuu8dgkWX_jZPuS28TchsfhyXIvp9LS5CSgV7Hsj1x5UqfU7tM3H-U0N/exec";  //TODO ← แก้ตรงนี้แล้วกด Run
  const res = UrlFetchApp.fetch(
...
// ============================================================
// 15. AUTO LOW STOCK ALERT (ตั้ง Trigger → checkAndAlertLowStock)
// ============================================================
function checkAndAlertLowStock() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (qty <= LOW_STOCK_LIMIT) {
      lowItems.push({ id: row[0], name: row[2], qty: qty });
    }
  }

  if (lowItems.length === 0) return; // ไม่มีของใกล้หมด ไม่ต้องแจ้ง
...
// ============================================================
// 16. INSTALL EDIT TRIGGER (รันครั้งเดียว!)
// ============================================================

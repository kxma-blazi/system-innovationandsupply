...
function doGet(e) {
  ...
  // 🎯 ดึงค่าตามที่พิมพ์ในเซลล์ (DisplayValues จะช่วยข้ามพวก Object รูปภาพได้ระดับหนึ่ง)
  ...
  for (let i = 1; i < data.length; i++) {
    let matCode = data[i][2] ? data[i][2].toString().trim() : ""; // Column C (Index 2)
    if (matCode === "" || matCode.toLowerCase().includes("cellimage")) {
      matCode = "NO-DATA";
    }

    // ระบบ Search (ถ้ามีคำค้นหา ให้กรองจาก Code หรือ Description)
    if (search) {
      const kw = search.toLowerCase();
      const desc = data[i][3].toLowerCase();
      if (!matCode.toLowerCase().includes(kw) && !desc.includes(kw)) continue;
    }

    const qty = Number(data[i][5]) || 0; // Column F (Index 5)
    if (qty <= 0) outCount++;
    else if (qty <= LOW_STOCK_LIMIT) lowCount++;
  ...
  // เตรียมส่งข้อมูลไปที่ไฟล์ Index.html
  const tmpl = HtmlService.createTemplateFromFile("Index");
  tmpl.page = page;
  tmpl.search = search;
  tmpl.stockRows = buildHtmlRows(rows); // ฟังก์ชันสร้าง <tr>...</tr>
  tmpl.totalItems = rows.length;
...
// ============================================================
// 🚩 ฟังก์ชันสร้างแถว HTML ตาราง STOCK
// ============================================================
function buildHtmlRows(rows) {
  if (rows.length === 0) {
    return `<tr><td colspan="10" style="text-align:center;">ไม่พบข้อมูล</td></tr>`;
  }

  return rows
    .map((r) => {
      // ป้องกันการใส่ตัวอักษรพิเศษใน URL QR Code
      const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(r.code)}`;

      // ตั้งค่าคลาสสีให้ป้ายสถานะ
      const badgeHtml =
        r.qty <= 0
      ...
      <td style="text-align:center;"><img src="${qrUrl}" width="35" height="35" style="border-radius:4px; cursor:pointer;" onclick="window.open('${qrUrl}')"></td>
      <td>${r.desc}</td>
...
/**
 * ดึงข้อมูล SERIAL_DATA (Delivery Records)
 */
function getDeliveryRecords(limit = 100) {
  try {
    ...
    Logger.log("Error getDeliveryRecords: " + e);
    return { records: [] };
  }
}
...
function getStockAlerts() {
  try {
    ...
    const rawData = stockSheet.getRange(1, 1, lastRow, 11).getDisplayValues();

    const alerts = [];

    for (let i = 1; i < rawData.length; i++) {
      let matCode = rawData[i][2] ? String(rawData[i][2]).trim() : "";
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (
        ...
        sheet.getRange(i + 1, 6, 1, 1).setValue(newTotal);
        sheet.getRange(i + 1, colIndex + 1, 1, 1).setValue(newDeptTotal);

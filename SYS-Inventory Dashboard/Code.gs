...
// 1. CONFIGURATION
// ============================================================
const TELEGRAM_TOKEN  = "8327163778:AAF0djHE5WszUTd3XcqVaqLKSVA9PiZ-zTo";
...
function withdraw(code, qty, colIndex, deptName, user, targetSheetName) {
  ...
  for (let i = 1; i < data.length; i++) {
    // ตรวจสอบรหัส (Case-insensitive)
    if (data[i][0] && data[i][0].toString().toLowerCase() === code.toLowerCase()) {
      
      const cur = Number(data[i][5]); // คอลัมน์ F (Index 5) คือ QTY รวม
      const itemName = data[i][3];   // คอลัมน์ D (Index 3) คือ Description
      
      if (cur < qty) return `❌ *สต็อกไม่พอ*\nรายการ: ${itemName}\nคงเหลือ: ${cur} ชิ้น`;
      
      const newQty = cur - qty;
      const newDeptValue = (Number(data[i][colIndex]) || 0) + qty;
      
      // อัปเดตค่าลง Sheet (แถวต้อง +1, คอลัมน์ต้อง +1 เพราะ Sheet เริ่มนับที่ 1)
      sheet.getRange(i + 1, 6).setValue(newQty);             // อัปเดตช่อง QTY รวม (Col F)
      sheet.getRange(i + 1, colIndex + 1).setValue(newDeptValue); // อัปเดตช่องแผนกที่เลือก
      
      // บันทึกประวัติลงชีต Logs
      writeLog(user, "Withdraw", data[i][0], qty, deptName, newQty);
...
/**
 * ฟังก์ชันรับคำสั่งจาก Dashboard เพื่อไปเรียกใช้ระบบตัดสต็อกเดิม
 */
function updateStockFromDashboard(id, actionType, qty, dept) {
  const user = "Web Dashboard"; // ระบุชื่อผู้ทำรายการเพื่อให้รู้ว่ามาจากหน้าเว็บ
  const sheetName = "STOCK";
  const qtyNum = Number(qty);
  
  if (actionType === 'withdraw') {
    // กำหนด Index คอลัมน์ของแต่ละแผนก (CBR=6, CCS=7, SKO=8, RYG=9, TRT=10)
    const colMap = { CBR: 6, CCS: 7, SKO: 8, RYG: 9, TRT: 10 };
    const col = colMap[dept.toUpperCase()] || 6;
    
    // เรียกใช้ฟังก์ชัน withdraw ที่คุณมีอยู่แล้ว (มันจะไปเรียก writeLog ให้เอง)
    return withdraw(id, qtyNum, col, dept.toUpperCase(), user, sheetName);
  } else {
    // เรียกใช้ฟังก์ชัน restock ที่คุณมีอยู่แล้ว (มันจะไปเรียก writeLog ให้เอง)
    return restock(id, qtyNum, user, sheetName);
  }
}

// ============================================================
// 17. GET LOGS DATA (สำหรับแสดงผลบน Dashboard)
// ============================================================
function getLogsData() {
  try {
    const ss = getSpreadsheet();
    // ตรวจสอบชื่อชีตให้ตรงกับที่คุณใช้ (ใน writeLog คุณใช้ "Logs")
    const sheet = ss.getSheetByName("Logs"); 
    
    if (!sheet) {
      console.error("ไม่พบชีต Logs");
      return { logs: [], error: "ไม่พบชีต Logs" };
    }
    ...
    const logs = data.map(row => ({
      timestamp: row[0] instanceof Date ? Utilities.formatDate(row[0], "GMT+7", "dd/MM/yy HH:mm") : String(row[0]),
